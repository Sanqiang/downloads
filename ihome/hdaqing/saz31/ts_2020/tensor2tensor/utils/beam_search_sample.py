...
def beam_search(symbols_to_logits_fn,
  ...
  def grow_topk(i, alive_seq, alive_log_probs, states):
    ...
    def greedy_sample(flat_curr_scores):
        topk_scores, topk_ids = tf.nn.top_k(flat_curr_scores, k=beam_size * 2)
        return topk_scores, topk_ids

    def multinomial_sample(flat_curr_scores):
        flat_curr_probs = tf.nn.softmax(flat_curr_scores)
        one_hot_k_samples = tf.distributions.Multinomial(
            total_count=1., probs=flat_curr_probs).sample(beam_size * 2)
        k_argmax = tf.argmax(one_hot_k_samples, axis=2, output_type=tf.int32)
        topk_ids = tf.transpose(k_argmax)
        indices = tf.stack([batch_pos, topk_ids], axis=-1)
        topk_scores = tf.gather_nd(flat_curr_scores, indices)
        return topk_scores, topk_ids

    topk_scores, topk_ids = tf.cond(
        tf.greater(tf.random_uniform([]), 0.1),
        lambda: greedy_sample(flat_curr_scores),
        lambda: multinomial_sample(flat_curr_scores))
