...
def transformer_decoder_parallel(decoder_input,
                        encoder_output,
                        decoder_self_attention_bias,
                        encoder_decoder_attention_bias,
                        hparams,
                        ...
                        external_bias=None,
                        external_output2=None,
                        external_bias2=None,
                        external_output3=None,
                        external_bias3=None,
                        shared_weight=False):
  """A stack of transformer layers.
  ...
  with tf.variable_scope(name):
    for layer in range(hparams.num_decoder_layers or hparams.num_hidden_layers):
      ...
      with tf.variable_scope(layer_name, reuse=tf.AUTO_REUSE):
        ...
        #     x = common_layers.layer_postprocess(x, y, hparams)
        #
        # if external_output2 is not None:
        #   with tf.variable_scope("external_attention2"):
        #     y = common_attention.multihead_attention(
        #           common_layers.layer_preprocess(x, hparams),
        #           external_output2,
        #           external_bias2,
        ...
        #     x = common_layers.layer_postprocess(x, y, hparams)
        #
        # if external_output3 is not None:
        #   with tf.variable_scope("external_attention3"):
        #     y = common_attention.multihead_attention(
        #           common_layers.layer_preprocess(x, hparams),
        #           external_output3,
        #           external_bias3,
        ...
        parallel_names, parallel_list, parallel_bias_list = [], [], []
        if hist_vector is not None:
            parallel_names.append("hist_attention")
            parallel_list.append(hist_vector)
            parallel_bias_list.append(tf.zeros([1, 1, 1, 1]))
        if external_output is not None:
            parallel_names.append("external_attention")
            parallel_list.append(external_output)
            parallel_bias_list.append(external_bias)
        if external_output2 is not None:
            parallel_names.append("external_attention2")
            parallel_list.append(external_output2)
            parallel_bias_list.append(external_bias2)
        if external_output3 is not None:
            parallel_names.append("external_attention3")
            parallel_list.append(external_output3)
            parallel_bias_list.append(external_bias3)
        if parallel_names:
            x_outs = []
            for p_id, name in enumerate(parallel_names):
                with tf.variable_scope(name):
                    y = common_attention.multihead_attention(
                        common_layers.layer_preprocess(x, hparams),
                        parallel_list[p_id],
                        parallel_bias_list[p_id],
                        hparams.attention_key_channels or hparams.hidden_size,
                        hparams.attention_value_channels or hparams.hidden_size,
                        hparams.hidden_size,
                        hparams.num_heads,
                        ...
                        max_length=hparams.get("max_length"),
                        vars_3d=hparams.get("attention_variables_3d"))
                    x_out = common_layers.layer_postprocess(x, y, hparams)

                    x_gate = tf.layers.dense(
                        x_out,
                        x_out.shape.as_list()[-1],
                        activation=tf.nn.tanh,
                    )
                    x_outs.append(x_out * x_gate)

            x = tf.add_n(x_outs)

        if npad_mode == 'v1' and layer == ((hparams.num_decoder_layers or hparams.num_hidden_layers) - 1):
            with tf.variable_scope("npad"):
                x = common_layers.layer_preprocess(x, hparams)
                hidden_size = x.get_shape()[-1].value
                npad_w = tf.get_variable(
                        'npad_w', shape=[1, hidden_size, hidden_size],
                        initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32)
                x = tf.nn.conv1d(x, npad_w, 1, 'SAME')
...
def transformer_multi_decoder(decoder_input,
                        encoder_output,
                        decoder_self_attention_bias,
                        encoder_decoder_attention_bias,
                        encoder_output2, encoder_decoder_attention_bias2,
                        hparams,
                        cache=None,
                        decode_loop_step=None,
                        name="decoder",
      ...
      with tf.variable_scope(layer_name):
        with tf.variable_scope("self_attention"):
          y = common_attention.multihead_attention(
              ...
              decode_loop_step=decode_loop_step,
              vars_3d=hparams.get("attention_variables_3d"))
          x0 = common_layers.layer_postprocess(x, y, hparams)

        if hist_vector is not None:
            with tf.variable_scope("hist_attention"):
                y = common_attention.multihead_attention(
                    common_layers.layer_preprocess(x0, hparams),
                    hist_vector,
                    tf.zeros([1, 1, 1, 1]),
                    hparams.attention_key_channels or hparams.hidden_size,
                    hparams.attention_value_channels or hparams.hidden_size,
                    ...
                    max_length=hparams.get("max_length"),
                    vars_3d=hparams.get("attention_variables_3d"))
                x0 = common_layers.layer_postprocess(x0, y, hparams)
        if encoder_output is not None:
          with tf.variable_scope("encdec_attention"):
            y = common_attention.multihead_attention(
                common_layers.layer_preprocess(x0, hparams),
                ...
                max_length=hparams.get("max_length"),
                vars_3d=hparams.get("attention_variables_3d"))
            x1 = common_layers.layer_postprocess(x0, y, hparams)
        if encoder_output2 is not None:
          with tf.variable_scope("encdec_attention2"):
            if not direct_mode or direct_mode == 'stack':
              print('Use Stack mode for transformer_multi_decoder')
              y = common_attention.multihead_attention(
                common_layers.layer_preprocess(x1, hparams),
                encoder_output2,
                encoder_decoder_attention_bias2,
                hparams.attention_key_channels or hparams.hidden_size,
                hparams.attention_value_channels or hparams.hidden_size,
                hparams.hidden_size,
                hparams.num_heads,
                ...
                max_length=hparams.get("max_length"),
                vars_3d=hparams.get("attention_variables_3d"))
              x1 = common_layers.layer_postprocess(x1, y, hparams)
            elif direct_mode == 'gate':
              print('Use Gate mode for transformer_multi_decoder')
              y = common_attention.multihead_attention(
                  common_layers.layer_preprocess(x0, hparams),
                  encoder_output2,
                  encoder_decoder_attention_bias2,
                  ...
                  max_length=hparams.get("max_length"),
                  vars_3d=hparams.get("attention_variables_3d"))
              x1_stack = common_layers.layer_postprocess(x0, y, hparams)
              evidence = tf.concat([x1, x1_stack], axis=-1)
              hidden_size = x1.get_shape()[-1].value
              direct_gate_w = tf.get_variable('direct_gate_w', shape=(
                  1, hidden_size*2, hidden_size))
              direct_gate_b = tf.get_variable('direct_gate_b', shape=(
                  1,  1, hidden_size))
              gate = tf.sigmoid(
                  tf.nn.conv1d(evidence, direct_gate_w, 1, 'SAME') + direct_gate_b)
              x1 = x1 * gate + x1_stack * (1 - gate)
        ...
        with tf.variable_scope("ffn"):
          y = transformer_ffn_layer(
              ...
              cache=layer_cache,
              decode_loop_step=decode_loop_step)
          x = common_layers.layer_postprocess(x1, y, hparams)
    # if normalization is done in layer_preprocess, then it should also be done
    # on the output, since the output can grow very large, being the sum of
    # a whole stack of unnormalized layer outputs.
    return common_layers.layer_preprocess(x, hparams)
...
def transformer_decoder_contexts(decoder_input,
                        encoder_output,
                        decoder_self_attention_bias,
                        encoder_decoder_attention_bias,
                        hparams,
