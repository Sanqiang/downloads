...
def compute_topk_scores_and_seq(sequences, scores, scores_to_gather, flags,
                                beam_size, batch_size, topk_seq_logitl, prefix="default",
  ...
  with tf.device('/device:CPU:0'):
    topk_seq_logitl = gather(topk_seq_logitl, "_topk_seq_logitl")
  return topk_seq, topk_gathered_scores, topk_flags, topk_gathered_states, topk_seq_logitl
...
def beam_search(symbols_to_logits_fn,
  ...
  alive_seq_logit = tf.zeros([batch_size, beam_size, 1, vocab_size], tf.float32)
  finished_seq_logit = tf.zeros(common_layers.shape_list(alive_seq_logit), tf.float32)

  def grow_finished(finished_seq, finished_scores, finished_flags, finished_seq_logitl,
                    curr_seq, curr_scores, curr_finished, curr_seq_logitl):
    """Given sequences and scores, will gather the top k=beam size sequences.
    ...
    with tf.device('/device:GPU:2'):
        ...
        with tf.device('/device:CPU:0'):
          curr_seq_logitl = tf.concat([finished_seq_logitl, curr_seq_logitl], axis=1)
        return compute_topk_scores_and_seq(
            curr_finished_seq, curr_finished_scores, curr_finished_scores,
            curr_finished_flags, beam_size, batch_size, curr_seq_logitl,
            "grow_finished")

  def grow_alive(curr_seq, curr_scores, curr_log_probs, curr_finished, states, topk_seq_logitl):
    """Given sequences and scores, will gather the top k=beam size sequences.
  ...
  def grow_topk(i, alive_seq, alive_log_probs, states, alive_seq_logitl):
    ...
    # Append seq logit
    topk_seq_logitl = tf.gather_nd(alive_seq_logitl, topk_coordinates)
    topk_seq_logitl = tf.concat(
        [topk_seq_logitl, tf.expand_dims(tf.gather_nd(curr_scores, topk_coordinates), axis=2)], axis=2)

    topk_finished = tf.equal(topk_ids, eos_id)

    return topk_seq, topk_log_probs, topk_scores, topk_finished, states, topk_seq_logitl

  def inner_loop(i, alive_seq, alive_log_probs, finished_seq, finished_scores,
                 finished_flags, states, alive_seq_logitl, finished_seq_logitl):
    ...
    topk_seq, topk_log_probs, topk_scores, topk_finished, states, topk_seq_logitl = grow_topk(
        i, alive_seq, alive_log_probs, states, alive_seq_logitl)
    alive_seq, alive_log_probs, _, states, alive_seq_logitl = grow_alive(
        topk_seq, topk_scores, topk_log_probs, topk_finished, states, topk_seq_logitl)
    finished_seq, finished_scores, finished_flags, _, finished_seq_logitl = grow_finished(
        finished_seq, finished_scores, finished_flags, finished_seq_logitl, topk_seq, topk_scores,
        topk_finished, topk_seq_logitl)
  ...
  def _is_finished(i, unused_alive_seq, alive_log_probs, unused_finished_seq,
                   ...
                   alive_seq_logitl, finished_seq_logitl):
  ...
  (_, alive_seq, alive_log_probs, finished_seq, finished_scores,
   finished_flags, _, alive_seq_logit, finished_seq_logit) = tf.while_loop(
       _is_finished,
       inner_loop, [
           tf.constant(0), alive_seq, alive_log_probs, finished_seq,
           finished_scores, finished_flags, states, alive_seq_logit, finished_seq_logit
       ],
       shape_invariants=[
           tf.TensorShape([]),
  ...
  finished_seq_logit.set_shape([None, beam_size, None, vocab_size])
  alive_seq_logit.set_shape([None, beam_size, None, vocab_size])
  ...
  finished_seq_logit = tf.where(
      tf.reduce_any(finished_flags, 1), finished_seq_logit, alive_seq_logit)
  return finished_seq, finished_seq_logit
