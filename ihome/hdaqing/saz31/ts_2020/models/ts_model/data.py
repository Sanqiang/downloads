...
class Data:
    ...
    def _parse(self, features, is_training):
        def _py_process_line_pair(
            ...
            src_wds, trg_wds = src_wds.decode().lower(), trg_wds.decode().lower()
            extra_vec = [-1]
                ...
                for l_id, template_comp in enumerate(template_comps):
                    for i, template_tk in enumerate(template_comp):
                        ...
                        template_comp_ids[l_id].extend(
                            [self.syntax_vocab.encode_token(template_tk) for _ in range(len(src_stacked_ids[i]))])
                ...
                for l_id, template_simp in enumerate(template_simps):
                    for i, template_tk in enumerate(template_simp):
                        ...
                        template_simp_ids[l_id].extend([
                            self.syntax_vocab.encode_token(template_tk) for _ in range(len(trg_stacked_ids[i]))])
