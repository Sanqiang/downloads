...
def SARIsent(ssent, csent, rsents, asreward=False):
    ...
    # finalscore = (avgkeepscore + avgdelscore + avgaddscore) / 3
    # print('add:%s' % avgaddscore)
    # print('del:%s' % avgdelscore)
    # print('keep:%s' % avgkeepscore)
...
# corpus_sari_script_nonref = '/zfs1/hdaqing/saz31/dataset/ts_script/corpus_sari_nonref.sh'
# corpus_sari_script = '/zfs1/hdaqing/saz31/dataset/ts_script/corpus_sari.sh'
# joshua_class = '/zfs1/hdaqing/saz31/dataset/ts_script/ppdb-simplification-release-joshua5.0/joshua/class'
corpus_sari_script_nonref = '/content/zfs1/hdaqing/saz31/dataset/ts_script/corpus_sari_nonref.sh'
corpus_sari_script = '/content/zfs1/hdaqing/saz31/dataset/ts_script/corpus_sari.sh'
joshua_class = '/content/zfs1/hdaqing/saz31/dataset/ts_script/ppdb-simplification-release-joshua5.0/joshua/class'
...
def get_sari_from_joshua(step, path_dst, path_ref, path_src, targets, resultdir, num_ref, prefix=""):
    ...
    if not os.path.exists(path_tar):
        f = tf.gfile.GFile(path_tar, 'w')
        # joshua require lower case
        f.write(result2txt(targets, lowercase=True))
        f.close()
    if num_ref > 0:
        return get_sari_result_joshua(path_ref, path_src, path_tar, num_ref)
    else:
        return get_sari_result_joshua_nonref(path_dst, path_src, path_tar)


def get_sari_result_joshua(path_ref, path_src, path_tar, num_ref):
    args = ' '.join(['bash', corpus_sari_script,
                     path_tar, path_ref, path_src,
                     joshua_class, str(num_ref)])
...
def get_sari_result_joshua_nonref(path_ref, path_src, path_tar):
    args = ' '.join(['bash', corpus_sari_script_nonref,
                     path_tar, path_ref, path_src,
                     joshua_class])
