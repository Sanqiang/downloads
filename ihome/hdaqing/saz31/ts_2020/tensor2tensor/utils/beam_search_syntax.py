...
def compute_topk_scores_and_seq(sequences, scores, scores_to_gather, flags,
  ...
  sequences_list = tf.unstack(sequences, axis=0)
  sequence_output = []
  for s_id, sequence in enumerate(sequences_list):
      sequence = gather(sequence, "_topk_%s_seq" % s_id)
      sequence_output.append(sequence)
  topk_seq = tf.stack(sequence_output, axis=0)
...
def beam_search(symbols_to_logits_fn,
  ...
  """
  batch_size = common_layers.shape_list(initial_ids)[1]
  all_level = syntax_level + 1
  ...
  alive_seq = tf.expand_dims(alive_seq, axis=-1)  # (batch_size, beam_size, 1)
  states = {}
  ...
  eos_id = tf.stack(
      [eos_syntax_id for _ in range(syntax_level)] + [eos_word_id], axis=0)
  ...
  def grow_finished(finished_seq, finished_scores, finished_flags, curr_seq,
    ...
    finished_seq = tf.concat(
        [finished_seq,
         tf.zeros([all_level, batch_size, beam_size, 1], tf.int32)], axis=3)

    # Set the scores of the unfinished seq in curr_seq to large negative
    # values
    curr_scores += (1. - tf.to_float(curr_finished)) * -INF
    # concatenating the sequences and scores along beam axis
    curr_finished_seq = tf.concat([finished_seq, curr_seq], axis=2)
    ...
    # print_op = tf.print("curr_finished_scores:", curr_finished_scores,
    #                     "argmax:", tf.argmax(curr_finished_scores, axis=-1),
    #                     "curr_finished_seq:", curr_finished_seq,
    #                     summarize=-1)
    # with tf.control_dependencies([print_op]):
    #     curr_finished_seq = tf.identity(curr_finished_seq)
  ...
  def grow_topk(i, alive_seq, alive_log_probs, states):
    ...
    Args:
      i: loop index
      alive_seq: Topk sequences decoded so far [all_level, batch_size, beam_size, i+1]
    ...
    prev_flat_ids = tf.reshape(alive_seq, [all_level, batch_size * beam_size, -1])
    # (batch_size*beam_size, length, dimension)
    prev_flat_emb = symbols_to_embedding_fn(prev_flat_ids, -1)

    cur_topk_seqs, topk_log_probss, topk_scoress, topk_idss = [], [], [], []
    for l_id in range(all_level):
      if l_id == syntax_level:
        cur_vocab_size = vocab_size
      else:
        cur_vocab_size = syntax_vocab_size

      # (batch_size * beam_size, decoded_length)
      flat_logits = symbols_to_logits_fn(prev_flat_emb, l_id)
      ...
      # (batch_size, beam_size*decoded_length)
      flat_curr_scores = tf.reshape(curr_scores, [-1, beam_size * cur_vocab_size])

      cur_topk_scores, topk_ids = tf.nn.top_k(flat_curr_scores, k=beam_size)
      # if l_id == syntax_level:
      #     cur_topk_scores += 999999999910000.0
      topk_scoress.append(cur_topk_scores)

      # Recovering the log probs because we will need to send them back
      cur_topk_log_probs = cur_topk_scores * length_penalty
      topk_log_probss.append(cur_topk_log_probs)

      # Work out what beam the top probs are in.
      topk_beam_index = topk_ids // cur_vocab_size
      topk_ids %= cur_vocab_size  # Unflatten the ids
      topk_idss.append(topk_ids)
      ...
      # bools
      # [all_level, batch_size, beam_size, i+1 => [batch_size, beam_size, i+1]
      cur_alive_seq = alive_seq[l_id, :]
      cur_topk_seq = tf.gather_nd(cur_alive_seq, topk_coordinates)

      # Append the most probable alive
      cur_topk_seq = tf.concat([cur_topk_seq, tf.expand_dims(topk_ids, axis=2)], axis=2)
      prev_flat_emb += symbols_to_embedding_fn(cur_topk_seq, l_id)
      cur_topk_seqs.append(cur_topk_seq)

      # Update for next interaction
      alive_log_probs = cur_topk_scores

      if l_id == 0:
        topk_finished = tf.equal(topk_ids, eos_syntax_id)

    # topk_finished = tf.reduce_all(
    #     tf.equal(tf.stack(topk_idss, axis=-1), eos_id), axis=-1)

    topk_seq = tf.stack(cur_topk_seqs, axis=0)
    topk_log_probs = tf.add_n(topk_log_probss)
    topk_scores = tf.add_n(topk_scoress)
    # print_op = tf.print("topk_scores:", topk_scores,
    #                     "argmax:", tf.argmax(topk_scores, axis=-1),
    #                     "curr_finished_seq:", topk_seq,
    #                     summarize=-1)
    # with tf.control_dependencies([print_op]):
  ...
  alive_seq.set_shape((all_level, None, beam_size, None))
  finished_seq.set_shape((all_level, None, beam_size, None))
