...
def _find_train_ckptfiles(path, is_delete, get_prefix=False):
    ...
    # if not exists(path):
    #     return None, -1
    steps = [int(f[len(ckpt_prefix):-5]) for f in tf.gfile.ListDirectory(path)
...
def _get_model_files(steps, path):
    ...
    for step in steps:
        ...
        model_files.extend([f for f in tf.gfile.ListDirectory(path)
                            if tf.gfile.Exists(os.path.join(path, f)) and f[:len(model_pref)] == model_pref])
    return model_files
