...
def beam_search(symbols_to_logits_fn,
  ...
  # Expand to beam_size (batch_size, beam_size, syntax_level)
  alive_log_probs = tf.tile(initial_log_probs, [batch_size, 1])

  # Expand each batch and state to beam_size
  alive_seq = _expand_to_beam_size(initial_ids, beam_size)
  alive_seq = tf.expand_dims(alive_seq, axis=2)  # (batch_size, syntax_level, beam_size, 1)
  if states:
  ...
  def grow_finished(finished_seq, finished_scores, finished_flags, curr_seq,
    ...
    finished_seq = tf.concat(
        [finished_seq,
         tf.zeros([batch_size, syntax_level, beam_size, 1], tf.int32)], axis=2)
  ...
  def grow_topk(i, alive_seq, alive_log_probs, states):
    ...
    Args:
      i: loop index
      alive_seq: Topk sequences decoded so far [batch_size, syntax_level, beam_size, i+1]
    ...
    alive_seq_transpose = tf.transpose(alive_seq, perm=[1, 2])
    flat_ids = tf.reshape(alive_seq_transpose, [batch_size * beam_size, syntax_level, -1])

    # (batch_size * beam_size, decoded_length)
    if states:
      flat_states = nest.map_structure(_merge_beam_dim, states)
      cur_word_logit, cur_syntax_logit, flat_states = symbols_to_logits_fn(flat_ids, i, flat_states)
      states = nest.map_structure(
          lambda t: _unmerge_beam_dim(t, batch_size, beam_size), flat_states)
    else:
      cur_logit = symbols_to_logits_fn(
          flat_ids, alive_log_probs, i)

    ### Process for word ###
    word_logits = tf.reshape(cur_word_logit, [batch_size, beam_size, -1])
    syntax_logits = tf.reshape(cur_syntax_logit, [batch_size, syntax_level, beam_size, -1])

    # Convert logits to normalized log probs
    candidate_word_log_probs = common_layers.log_prob_from_logits(word_logits)
    candidate_syntax_log_probs = common_layers.log_prob_from_logits(syntax_logits)

    # Multiply the probabilities by the current probabilities of the beam.
    # (batch_size, beam_size, vocab_size) + (batch_size, beam_size, 1)
    log_word_probs = candidate_word_log_probs + tf.expand_dims(alive_log_probs, axis=2)
    # (batch_size, syntax_level, beam_size, vocab_size) + (batch_size, 1, beam_size, 1)
    log_syntax_probs = candidate_syntax_log_probs + tf.expand_dims(tf.expand_dims(alive_log_probs, axis=1), axis=-1)

    length_penalty = tf.pow(((5. + tf.to_float(i + 1)) / 6.), alpha)

    curr_word_scores = log_word_probs / length_penalty
    curr_syntax_scores = log_syntax_probs / length_penalty
    # Flatten out (beam_size, vocab_size) probs in to a list of possibilities
    flat_word_curr_scores = tf.reshape(curr_word_scores, [-1, beam_size * vocab_size])
    flat_syntax_curr_scores = tf.reshape(curr_syntax_scores, [-1, syntax_level, beam_size * syntax_vocab_size])

    # (batch_size, 2*beam_size), (batch_size, 2*beam_size)
    topk_word_scores, topk_word_ids = tf.nn.top_k(flat_word_curr_scores, k=beam_size * 2)
    # (batch_size, syntax_level, 2*beam_size), (batch_size, syntax_level, 2*beam_size)
    topk_syntax_scores, topk_syntax_ids = tf.nn.top_k(flat_syntax_curr_scores, k=beam_size * 2)

    topk_syntax_scores = tf.reduce_sum(topk_syntax_scores, axis=1)
    # (batch_size, 4 * beam_size)
    topk_scores = tf.concat([topk_word_scores, topk_syntax_scores], axis=1)

    # Recovering the log probs because we will need to send them back
    topk_log_probs = topk_scores * length_penalty

    # Work out what beam the top probs are in.
    topk_beam_index1 = topk_word_ids // vocab_size
    topk_word_ids %= vocab_size  # Unflatten the ids

    topk_beam_index2 = topk_syntax_ids // syntax_vocab_size
    topk_syntax_ids %= syntax_vocab_size  # Unflatten the ids
    ...
    # that survived.
    batch_pos = compute_batch_indices(batch_size, beam_size * 4)
  ...
  alive_seq.set_shape((None, beam_size, syntax_level, None))
  finished_seq.set_shape((None, beam_size, syntax_level, None))
